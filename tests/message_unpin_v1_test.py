...
def test_message_pin_invalid_token(clear, register):
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0']})
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': -1, 'message_id': register['channel_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': 10, 'message_id': register['dm_msg_0']})
...
def test_message_pin_no_permissions_channel(clear, register):
    # Pin messages
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1']})
    # Christina tries to unpin the messages
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token1'], 'message_id': register['channel_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token1'], 'message_id': register['channel_msg_1']})
...
def test_message_pin_no_permissions_dm(clear, register):
    # Pin messages
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token1'], 'message_id': register['dm_msg_2']})
    # Unpin but member not Owner of dm
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token1'], 'message_id': register['dm_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token1'], 'message_id': register['dm_msg_1']})
    # Global owner but not owner of dm
    resp_2 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_2']})
    # Assert expected status code (403) AccesssError
    assert resp_0.status_code == 403
    assert resp_1.status_code == 403
...
def test_pin_invalid_message_id(clear, register):
    # Try to unpin message that doesn't exist
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': -1})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': 10})
...
def test_pin_not_member_channel(clear, register):
    ...
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1']})
    # Cengiz tries to unpin messages but hes not in the channel
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token2'], 'message_id': register['channel_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token2'], 'message_id': register['channel_msg_1']})
    # Assert expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400
...
def test_pin_not_member_dm(clear, register):
    ...
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1']})
    # Cengiz tries to unpin messages but hes not in the dm
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token2'], 'message_id': register['dm_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token2'], 'message_id': register['dm_msg_1']})
    # Assert expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400

# InputError (400), the message is not pinned (channels + dms)
def test_pin_already_pinned(clear, register):
    # Celina tries to unpin messages that arent pinned
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1']})
    resp_2 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0']})
    resp_3 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1']})
    # Assert expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400
...
def test_pin_successful_channel(clear, register):
    ...
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1']})
    # Celina unpins both messages in channel
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1']})
...
def test_pin_successful_dm(clear, register):
    ...
    requests.post(config.url + 'message/pin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1']})
    # Celina unpins both messages in channel
    resp_0 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0']})
    resp_1 = requests.post(config.url + 'message/unpin/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1']})
    # Expected status code (200), successful
    assert resp_0.status_code == 200
    assert resp_1.status_code == 200
    # Assert expected output
