...
def test_unreact_invalid_token(clear, register):
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': -1, 'message_id': register['dm_msg_0'], 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': 10, 'message_id': register['dm_msg_0'], 'react_id': 1})
    # Expected status code (403) AccessError
    assert resp_0.status_code == 403
    assert resp_1.status_code == 403

def test_unreact_invalid_message_id(clear, register):
# InputError (400), message_id does not exist
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': -1, 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': 10, 'react_id': 1})
...
def test_unreact_not_member_channel(clear, register):
    # Cengiz tries to unreact to a message in a channel hes not in
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token2'], 'message_id': register['channel_msg_0'], 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token2'], 'message_id': register['channel_msg_1'], 'react_id': 1})
    # Expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400

# InputError (400), authorised user not in dm
def test_unreact_not_member_dm(clear, register):
    # Cengiz tries to unreact to a message in a dm hes not in
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token2'], 'message_id': register['dm_msg_0'], 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token2'], 'message_id': register['dm_msg_1'], 'react_id': 1})
    # Expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400

# InputError (400), invalid react ID (currently only 1 is valid)
def test_unreact_invalid_react_id(clear, register):
    # Christina tries to unreact with react id 0 and 2
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token1'], 'message_id': register['dm_msg_0'], 'react_id': 0})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token1'], 'message_id': register['dm_msg_1'], 'react_id': 2})
    # Expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400

# InputError (400), the auth user hasn't reacted to the message (dm + channel)
def test_unreact_already_reacted(clear, register):
    # Celina tries to unreact to messages
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0'], 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1'], 'react_id': 1})
    resp_2 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0'], 'react_id': 1})
    resp_3 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1'], 'react_id': 1})
    # Expected status code (400) InputError
    assert resp_0.status_code == 400
    assert resp_1.status_code == 400
...
def test_unreact_successful_channel(clear, register):
    # Celina reacts to all messages in channels + dms
    requests.post(config.url + 'message/react/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0'], 'react_id': 1})
    requests.post(config.url + 'message/react/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1'], 'react_id': 1})
    requests.post(config.url + 'message/react/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0'], 'react_id': 1})
    requests.post(config.url + 'message/react/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1'], 'react_id': 1})
    # Christina reacts to one message in channel
    requests.post(config.url + 'message/react/v1', json={'token': register['token1'], 'message_id': register['channel_msg_1'], 'react_id': 1})
    # Celina unreacts (1) to all messages in channels and dms
    resp_0 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['dm_msg_0'], 'react_id': 1})
    resp_1 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['dm_msg_1'], 'react_id': 1})
    resp_2 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['channel_msg_0'], 'react_id': 1})
    resp_3 = requests.post(config.url + 'message/unreact/v1', json={'token': register['token0'], 'message_id': register['channel_msg_1'], 'react_id': 1})
